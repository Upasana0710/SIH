...
function ProfileCard({user}) {
  ...
  <h6 className={styles.user_email}>{user.email}</h6>
	...
	<p className={styles.user_bio}>Fueled by a passion for Learning,<br/>I'm an Engineering student </p>
	<div className={styles.buttons}>
	</div>
	<div className={styles.skills}>
    <div className={styles.skills2}>
		<li className={styles.heading}>My expertise
		<ul className={styles.courses}>
    {teachSub.map((course, index) => (
          <li className={styles.course_list} key={index}>{course}</li>
        ))}
			
		</ul>
    </li>
    <li className={styles.vertical_line}></li>
    <li className={styles.heading}>Looking for
		<ul className={styles.courses}>
